...
int main()
{
	...
	// , 
	RenderWindow window(VideoMode(resolution.x, resolution.y), "Social Zombie Arena");
	...
	spriteGameOver.setScale(resolution.x / 1280, resolution.y / 720);
	...
	spriteAmmoIcon.setPosition(28, resolution.y - 100);
	...
	pausedText.setOrigin(pausedTextRect.left + pausedTextRect.width / 2.0f, 
						 pausedTextRect.top + pausedTextRect.height / 2.0f);
	...
	levelUpText.setOrigin(levelUpTextRect.left + levelUpTextRect.width / 2.0f,
		levelUpTextRect.top + levelUpTextRect.height / 2.0f);
	levelUpText.setPosition(resolution.x / 2.0f, resolution.y / 2.0f);

	// Choosing up
	Text choosingText;
	choosingText.setFont(font);
	choosingText.setCharacterSize(60);
	choosingText.setFillColor(Color::White);
	std::stringstream choosingStream;
	choosingStream << "9- Twitter" << "\n8- Facebook" << "\n7- Instagram";
	choosingText.setString(choosingStream.str());

	// Position the text
	FloatRect choosingTextRect = choosingText.getLocalBounds();
	choosingText.setOrigin(choosingTextRect.left + choosingTextRect.width / 2.0f,
		choosingTextRect.top + choosingTextRect.height / 2.0f);
	choosingText.setPosition(resolution.x / 2.0f, resolution.y / 2.0f);
	...
	zombiesRemainingText.setPosition(resolution.x - 300, resolution.y - 100);
	...
	waveNumberText.setPosition(resolution.x - 560, resolution.y - 100);
	waveNumberText.setString("Wave: 0");

	// Health bar
	RectangleShape healthBar;
	healthBar.setFillColor(Color::Red);
	healthBar.setPosition(resolution.x - 980, resolution.y - 105);
		...
		*/
		...
		if (state == State::LEVELING_UP)
		{
			...
			if (state == State::PLAYING)
			{
				...
				player.spawn(arena, resolution, tileSize, playerImage);
		...
		if (state == State::CHOOSING)
		{
			...
			// Handle the player CHOOSING
			if (event.key.code == Keyboard::Num9)
			{
				playerImage = "twitter.png";
				state = State::LEVELING_UP;
			}

			if (event.key.code == Keyboard::Num8)
			{
				playerImage = "facebook.png";
				state = State::LEVELING_UP;
			}

			if (event.key.code == Keyboard::Num7)
			{
				playerImage = "instagram.png";
				state = State::LEVELING_UP;
			}
		}
